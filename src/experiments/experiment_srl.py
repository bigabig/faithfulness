...
precisions, recalls, f1s, alignments, similarities, summaries_phrases, sources_phrases, summaries_sentences, sources_sentences = metric.score_batch(summaries, sources, True).values()
...
for (idx, x) in enumerate(data):
    ...
    x[f"{metric_name}_new_summary_sentences"] = summaries_sentences[idx]
    x[f"{metric_name}_new_source_sentences"] = sources_sentences[idx]
